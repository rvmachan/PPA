...
crime_net <- 
  ...
  mutate(countBurglaries = 1) %>% # adding a column
  aggregate(., fishnet, sum) %>% #summarizing them and adding them up
...
final_net.nb <- poly2nb(as_Spatial(final_net), queen=TRUE) # is a queen matrix, is deciding who is a neighbor
...
rbind(burg_KDE_sf, burg_risk_sf) %>%
  ...
  ggplot() +
    ...
    #geom_sf(data = sample_n(burglaries18, 3000), size = .25, colour = "light grey") +
    facet_wrap(~label, ) +
