...
The housing market in Philadelphia is a reflection of the city's economy and social dynamics. There are various neighborhoods in the city with unique character, amenities, and housing styles. Philadelphia's diverse housing stock includes rowhouses, townhomes, apartments, and single-family homes, catering to different lifestyles and preferences.

This project evaluates factors affecting housing prices to create an accurate and generalizable OLS regression model to predict them. Housing prices in Philadelphia have been subject to fluctuations over the years, influenced by various factors such as the neighborhood, property type, and home condition. Sale prediction algorithms are crucial tools that help different stakeholders in the housing market make informed decisions, identify inequalities, and ensure the real estate market's overall health and fairness. This project builds on this theory and aims to evaluate the factors affecting housing prices to create an accurate and generalizable OLS regression model to predict the housing prices in Philadelphia.
...
p_load (sf, tidyverse, knitr, kableExtra, rmarkdown, tidycensus, dplyr, scales, stringr, ggcorrplot, readxl, data.table, tmap)
...
```{r corr int}
int_variables <- c("price", "condition", "sqft_living", "yr_built", "bedrooms", "bathrooms", "sqft_basement", "sqft_lot")
...
closestTransitStop <- sf::st_nearest_feature(seattleData, seattleTransitStops)

#converting into rsgeo geometries
x <- rsgeo::as_rsgeo(seattleData)
y <- rsgeo::as_rsgeo(seattleTransitStops)

#calculating distance
seattleData$dist2NearestTransitStop <- rsgeo::distance_euclidean_pairwise(x,y[closestTransitStop])
...
seattleTracts <- sf::st_join(seattleData, kcTracts_filtered, by = "TRACT") #trying to join it by tract only so we drop all the non-Seattle ones but it won't work dunno why
...
The average age for a house in Philadelphia is approximately 65 years indicating a large share of the market is held by homes constructed a long time ago.
...
stargazer(as.data.frame(DemographicVariables), type="text", digits=1, title = "Descriptive Statistics for Seattle Homes Demographic Variables", out = "Training_PHLSpatial.txt")
```
...
The average median household income in the observed areas is $108,185.2. However, approximately 8.4% of the population in the observed areas falls below the poverty level, on average.
...
#mapping internal variables (condition, basement presence, house size, price)

#mapping sale price
ggplot() +
  geom_sf(data= kcTracts, fill = "grey89", color = "darkgrey") +
  geom_sf(data = seattleData, aes(colour = q5(price))) +
  scale_fill_manual(values = c('red', 'orange', 'lightyellow', 'lightblue', 'skyblue3'),
                      labels = qBr(seattleData, "price"),
                      name = "Quintile Breaks :/n Sale Price",
                      na.value = NA) +
  labs(title = "Properties by Sale Price", subtitle = "Philadelphia 2022-23",
       caption = "Figure 3.1.1") +
  mapTheme()

#IT DOES NOT WORK AND I DO NOT KNOW WHY AND IM 99% SURE ITS A SMALL DUMB THING
...
ggplot()+
  ...
  scale_colour_manual(values = palettee, name = "Condition Type") +
...
ggplot()+
  ...
  geom_sf(data = seattleData, aes(colour =(basementPresent)),
          show.legend = "point", size = .5) +
  scale_colour_manual(values = palettee, name = "Basement Presence") +
...
ggplot()+
  ...
  geom_sf(data = seattleData, aes(colour =(houseSize)),
          show.legend = "point", size = .5) +
  scale_colour_manual(values = palettee, name = "House Size") +
...
#can't do this until i can figure out why its not working
#problem is in the Scale_colour_manual - can't figure out what the issue is though

ggplot()+
  geom_sf(data = kcTracts, fill ="grey89", color = "darkgrey") +
  geom_sf(data = seattleData, aes(colour = q5(price)),
          show.legend = "point", size = .75, alpha =0.3) +
  geom_sf(data = seattleSchools, colour = "black", size = 1.5, alpha = 0.6) +
  scale_colour_manual(values = palettee,
                      labels = qBr(seattleData, "price"),
                      name = "Quintile Breaks :/nSale Prices",
                      na.value = NA) +
  labs(title = "Properties Around Schools", 
       subtitle = "Seattle 2015",
...
#mapping median income around sold properties - are we doing sold properties??
ggplot()+
  geom_sf(data = kcTracts, fill = "grey89", color = "darkgrey") +
  geom_sf(data = kcTracts, aes(fill = medIncome, color = "transparent", alpha = 0.5)+
            scale_fill_brewer(palette = "BuPu",
                              labels = qBr(kcTracts, "medIncome"),
                              name = "Median Income in Quintile Breaks"))+
  geom_sf(data = seattleData, aes(colour = price),
          show.legend = "point", size = .2)+
  scale_colour_manual(values = palettee,
                      labels = qBr(seattleData, "price"),
                      name = "Quintile Breaks in Sale Prices",
                      na.value = NA) +
  labs(title ="Median Income Around Sold Properties", 
       subtitle = "Seattle 2015",
       caption = "Figure 3.3.1") +
  mapTheme()


#mapping pct poverty around sold properties
ggplot()+
  geom_sf(data = kcTracts, fill = "grey89", color = "darkgrey") +
  geom_sf(data = kcTracts, aes(fill = pctPoverty, color = "transparent", alpha = 0.5)+
            scale_fill_brewer(palette = "BuPu",
                              labels = qBr(kcTracts, "medIncome"),
                              name = "Median Income in Quintile Breaks"))+
  geom_sf(data = seattleData, aes(colour = price),
