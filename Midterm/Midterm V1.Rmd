...
To assess how errors are distributed by neighborhoods (in this case, census tracts)
...
# Assuming SalePrice.AbsError is a numeric vector of absolute errors
seattle.test$color_factor <- cut(
  seattle.test$SalePrice.AbsError,
  breaks = quantile(seattle.test$SalePrice.AbsError, probs = 0:5/5, na.rm = TRUE),
  include.lowest = TRUE,
  labels = FALSE
)
...
```{r}
# Calculate quintiles for Price.AbsError and create a factor variable for coloring
quintiles <- quantile(seattle.test$SalePrice.AbsError, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
seattle.test$PriceQuintiles <- cut(seattle.test$SalePrice.AbsError, breaks = quintiles, labels = 1:5, include.lowest = TRUE,
                                   )

# Define your color palette
palette <- c("#d7191c", "#fdae61", "#ffffbf", "#abd9e9", "#2c7bb6")

# Plot the map with the categorized Price.AbsError
ggplot() +
  geom_sf(data = kcTracts_filtered, fill = "grey89") +
  geom_sf(data = seattle.test, aes(colour = PriceQuintiles), show.legend = "point", size = .75) +
  scale_colour_manual(values = palette, name="Quintile Breaks") +
  labs(
    title = "Map of Price Absolute Errors for Test Set",
    subtitle = "Seattle"
  ) +
  theme_void()
```
...
seattle.test$PriceQuintiles <- cut(
  seattle.test$SalePrice.AbsError,
  breaks = quantile(seattle.test$SalePrice.AbsError, probs = seq(0, 1, by = 0.2), na.rm = TRUE),
  include.lowest = TRUE,
  labels = c("1st Quintile", "2nd Quintile", "3rd Quintile", "4th Quintile", "5th Quintile")
)

# Define your palette (ensure it has 5 colors for the 5 quintiles)
palette <- c("#d7191c", "#fdae61", "#ffffbf", "#abd9e9", "#2c7bb6")

ggplot() +
  geom_sf(data = kcTracts_filtered, fill = "grey89") +
  geom_sf(data = seattle.test, aes(color = PriceQuintiles), size = 0.75) +
  scale_color_manual(values = palette, name="Price Quintiles") +
  labs(
    title = "Predicted Sale Price for Test Set",
    subtitle = "Seattle"
  ) +
  theme_void()
...
seattle_residuals <- merge(seattle.test, kcTracts_filtered, by.x = "TRACT.x", by.y = "TRACT")
