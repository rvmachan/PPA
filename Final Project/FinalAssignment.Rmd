---
title: 'Assignment 3: Predictive Policing'
author: "KRC change 2"
date: "04/14/2024"
output:
  html_document:
...
I'm going to run through the code base with a few variables in the model.
...
4. Compare model performance to Kernal Density.
...
p_load(sf, tidyverse, tidycensus, RSocrata, viridis, spatstat, raster, spdep, FNN, grid, gridExtra, knitr, kableExtra, classInt)
...
This uses the Socrata package for some data sets. I am bringing in assault data for this assignment.
...
vacantBuilding <-
  ...
  na.omit() %>%
  st_transform('ESRI:102728') %>%
  st_transform(st_crs(philFishnet)) %>%
...
tracts22 <- tracts22 %>% st_transform(st_crs(phillyFishnet))
...
```{r police data}
# Read and process assault data
assault <- 
  read.socrata("https://data.cityofchicago.org/Public-Safety/Crimes-2017/d62x-nvdr") %>% 
  filter(Primary.Type == "ASSAULT") %>%  # Filter assault data
...
grid.arrange(
  ...
  ggplot() + 
    ...
    geom_sf(data = assault, colour = "red", size = 0.1, show.legend = "point") +  # Overlay assault crimes
    labs(title = "Assault, Chicago - 2017") +  # Set plot title
    theme_void(),  # Use a blank theme
  
  # Plot 2: Density of assault crimes with contours overlaid on Chicago boundary
  ggplot() + 
    geom_sf(data = chicagoBoundary, fill = "grey40") +  # Add Chicago boundary with grey fill
    stat_density2d(data = data.frame(st_coordinates(assault)),  # Compute 2D kernel density estimate
    ...
    labs(title = "Density of Assault crimes") +  # Set plot title
...
I'm using sf package to create the grid and using the boundary to clip the grid to the data.
...
I pulled in a single variable for the model to keep it simple. I looked at alley lights out data in relation to assault crime because poorly lit areas may have a higher incidence of such incidents, suggesting a possible correlation between street lighting and public safety.

```{r addpredictor}
## only pulling a single variable for our model to keep it simple
## using Socrata again
# Read the dataset of alley lights out 311 service requests from the City of Chicago
lightsout <- 
...
```
### Alley lights out overlaid on Chicago neighborhooods

```{r plot2}
  # Plot 2: Alley lights out overlaid on Chicago neighborhooods
  ggplot() + 
    geom_sf(data = neighborhoods) +  # Add Chicago boundary
    geom_sf(data = lightsout, colour = "red", size = 0.1, show.legend = "point") +  
    labs(title = "Alley Lights Out, Chicago - 2017") +  # Set plot title
    theme_void()  # Use a blank theme
```
...
The overall objective here was to assign the fishnet ID to each lights out point, group the points by fishnet ID and count them per ID, join that count back to the fishnet and then go from a long format to a wide format.


```{r jointofishnet}

# Join the alley lights out data with the fishnet grid based on spatial intersection
vars_net <- lightsout %>%  # Start with the alley lights out data
  ...
  group_by(uniqueID, Legend) %>%  # Group data by uniqueID and Legend (e.g., lights_out)
...
# Create nearest neighbor (NN) relationship from alley lights out data to fishnet grid cells
vars_net <- vars_net %>%  # Start with the summarized variables data
    mutate(lightsout.nn = nn_function(  # Create a new column for nearest neighbor information
        st_c(st_coid(vars_net)),  # Calculate centroids of fishnet grid cells
        st_c(lightsout),         # Get coordinates of abandoned cars
