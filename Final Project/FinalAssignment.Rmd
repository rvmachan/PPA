...
p_load(sf, tidyverse, tidycensus, RSocrata, viridis, spatstat, raster, spdep, FNN, grid, gridExtra, knitr, kableExtra, classInt, RColorBrewer, ggcorrplot, dplyr)
...
```{r - map theme - ideally we replace this later, include = FALSE}
...
<<<<<<< Updated upstream
```{r police data, results = 'hide'}
=======
```{r permits and data cleaning}
>>>>>>> Stashed changes
#Data Cleaning!
...
```{r error, results = 'hide', warning = FALSE, message = FALSE}
...
```{r kdConstruction, warning = FALSE, message = FALSE}
...
## Validating for a different time / city

```{r new, results = 'hide', message = FALSE, warning = FALSE}
...
```{r comp, warning = FALSE, message = FALSE}
...
```{r comp plot, warning = FALSE, message = FALSE}
...
```{r comp bar, message = FALSE, warning = FALSE }
#rbind(as_KDE_sf, as_risk_sf) %>%
#  st_drop_geometry() %>%
#  na.omit() %>%
...
This model can be used to inform planning decisions that are carried out by the Office of Innovation and Technology and the Smart Cities office. 
