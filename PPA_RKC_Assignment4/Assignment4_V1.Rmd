...
#this might be able to be taken out
rDivismTest <- rDivismTest %>%
  mutate(class = rDivismTest$recidivism_within_3years,
          Outcome = as.factor(rDivismTest$rDivismNumeric),
          Probs = predict(rDivismModel, rDivismTest, type= "response"))
...
cvFit <- train(recidivism_within_3years ~ .,
               data = rDivism %>% 
               dplyr::select(-prison_offense, -gender, -prior_arrest_episodes_felony, -rDivismNumeric),
               method = "glm", 
               family = "binomial",
               metric = "ROC", 
               trControl = ctrl)
