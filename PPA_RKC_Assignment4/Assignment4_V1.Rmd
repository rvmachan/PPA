...
#  mutate(ageTimeFrame = case_when(age_at_release <= 32 "18-22" & "23-27" & "28-32" ~ "Young",
 #                                 age_at_release == "33-37" & "38-42" ~ "Middle Aged", 
  #                                age_at_release == "43-47" & "48 or older" ~ "Old"))
...
rDivismModel <- glm(rDivismNumeric ~ ., #glm is generalized linear model, taking in the defaults
                    data=rDivismTrain %>% 
                    dplyr::select(-prison_offense, -ageTimeFrame, -recidivism_within_3years),
...
```{r second_model}
rDivismModel <- glm(rDivismNumeric ~ .,
                  data=rDivismTrain %>% dplyr::select(-Country, -Timestamp, -click, -userTimeFrame, -Fashion_FB_Group),
                  family="binomial" (link="logit"))
...
pR2(advertisingModel) #McFadden is pseudo R squared
...
testProbs <- data.frame(Outcome = as.factor(rDivismTest$rDivismNumeric),
