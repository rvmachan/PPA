...
```{r chunk4}
lm.fit <- lm(medv ~ lstat, data = Boston) #<- sometimes is bad for reproducibility and can be a bad habit
attach(Boston) #skip
...
summary(lm.fit)
```
#can tell the estimate is v far away from zero bcuz t value is big, the stars align, rsquared is over 50 saying that over 50% of the data can be explained by the relationship b/w the variables
...
```{r chunk6}
names(lm.fit)
coef(lm.fit) #why safer to use extractor functions vs by names?
```
...
```{r chunk11}
par(mfrow = c(2, 2)) #sets us up to plot 4 plots on one 
plot(lm.fit)
```
